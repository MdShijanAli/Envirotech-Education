...
const HomeBlog = () => {
    ...
    useEffect(() => {
        fetch('https://assignment-10-server-beta.vercel.app/home-blog')
            .then(res => res.json())
                    ...
                    <div className="p-5">
                        <div className="relative">
                            <Link to='/blog' className="block aspect-w-4 aspect-h-3">
                                <img className="object-cover w-full h-full" src={blg?.photo} alt="" />
                            </Link>
                        ...
                        <p className="mt-5 text-2xl font-semibold">
                            <Link to='/blog' className="text-black">{blg?.name} </Link>
                        </p>
                        <p className="mt-4 text-base text-gray-600">{blg?.description.slice(0, 250)}</p>
                        <Link to='/blog' className="inline-flex items-center justify-center pb-0.5 mt-5 text-base font-semibold text-blue-600 transition-all duration-200 border-b-2 border-transparent hover:border-blue-600 focus:border-blue-600">
                            Continue Reading
